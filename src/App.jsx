...
// ─── ASISTENT ────────────────────────────────────────────────
function getAdvice(totals, goals, dayData) {
  const tips = [];
  const calLeft = goals.calories - totals.calories;
  const hour = new Date().getHours();
  const mealCount = dayData.meals.length;
  const waterLeft = goals.water - (dayData.water || 0);
  const proteinLeft = goals.protein - totals.protein;
  const fiberLeft = goals.fiber - totals.fiber;
  const calPct = totals.calories / goals.calories;

  // Salut contextual
  if (mealCount === 0) {
    if (hour < 10) tips.push({ icon: "🌅", text: "Bună dimineața, boss! Nu ai înregistrat nimic încă. Începe cu un mic dejun bogat în proteine — ouă, iaurt grecesc sau shake proteic." });
    else if (hour < 14) tips.push({ icon: "☀️", text: "Boss, e deja prânz și n-ai mâncat nimic înregistrat. Nu uita să mănânci — săritul meselor încetinește metabolismul." });
    else tips.push({ icon: "🌙", text: "Boss, seara și încă zero mese înregistrate azi. Dacă ai mâncat și uitat să adaugi, fă-o acum pentru un istoric corect." });
    return tips;
  }

  // Calorii
  if (calLeft > 800) {
    tips.push({ icon: "🔥", text: `Boss, mai ai ${calLeft} kcal disponibile azi. Ai spațiu pentru o masă consistentă — ideal ceva cu proteine și legume.` });
  } else if (calLeft > 400) {
    tips.push({ icon: "⚡", text: `Bine mergi! Mai ai ${calLeft} kcal. Ajunge pentru o masă ușoară sau o gustare proteică.` });
  } else if (calLeft > 100) {
    tips.push({ icon: "🎯", text: `Aproape de obiectiv — mai ai doar ${calLeft} kcal. O gustare mică și ești perfect pentru azi.` });
  } else if (calLeft <= 0) {
    tips.push({ icon: "🛑", text: `Boss, ai depășit obiectivul cu ${Math.abs(calLeft)} kcal. Evită să mai mănânci azi, sau fă puțină mișcare.` });
  }

  // Proteine
  if (proteinLeft > 50) {
    tips.push({ icon: "💪", text: `Proteinele sunt scăzute — mai ai nevoie de ${Math.round(proteinLeft)}g. La urmatoarea masă alege carne slabă, ton, ouă sau brânză de vaci.` });
  } else if (proteinLeft < 0) {
    tips.push({ icon: "✅", text: `Proteinele sunt acoperite pentru azi — ${totals.protein}g din ${goals.protein}g. Bine făcut!` });
  }

  // Fibre
  if (fiberLeft > 15) {
    tips.push({ icon: "🥦", text: `Fibrele sunt mici azi — mai ai nevoie de ${Math.round(fiberLeft)}g. Adaugă legume, fructe sau leguminoase la următoarea masă.` });
  }

  // Apă
  if (waterLeft > 1000) {
    tips.push({ icon: "💧", text: `Boss, hidratarea e slabă — mai ai ${waterLeft}ml de băut. Încearcă să bei un pahar acum.` });
  } else if (waterLeft <= 0) {
    tips.push({ icon: "💧", text: `Hidratare excelentă azi — obiectivul de apă e atins!` });
  }

  // Ora zilei + mese
  if (hour >= 20 && calPct < 0.7) {
    tips.push({ icon: "🌙", text: `E seară și ai consumat doar ${Math.round(calPct * 100)}% din calorii. Dacă nu mai mănânci, poate fi prea puțin pe termen lung.` });
  }
  if (hour < 12 && mealCount === 0) {
    tips.push({ icon: "🍳", text: `Nu ai înregistrat micul dejun. Un mic dejun bun îți dă energie și reduce pofta la prânz.` });
  }
  if (mealCount >= 4) {
    tips.push({ icon: "👏", text: `${mealCount} mese înregistrate azi — constanță bună! Mesele dese mențin metabolismul activ.` });
  }

  // Grăsimi
  if (totals.fat > goals.fat * 1.3) {
    tips.push({ icon: "⚠️", text: `Grăsimile au depășit obiectivul (${totals.fat}g din ${goals.fat}g). La mesele următoare alege preparate la grătar, fierte sau la cuptor.` });
  }

  return tips.slice(0, 3); // max 3 sfaturi
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
      <svg width={w + 12} height={h + 20} viewBox={`0 0 ${w + 12} ${h + 20}`}>
        ...
        <clipPath id="bc"><rect x={6} y={8} width={w} height={h} rx={8} /></clipPath>
        <rect x={6} y={8 + h * (1 - pct)} width={w} height={h * pct} fill="url(#wg)" clipPath="url(#bc)"
...
function FoodModal({ onAdd, onClose }) {
  ...
  function handleAdd() {
    ...
    onAdd({
      ...
      calories: Math.round(selected.cal * ratio),
      protein: Math.round(selected.p * ratio * 10) / 10,
      carbs: Math.round(selected.c * ratio * 10) / 10,
      fat: Math.round(selected.f * ratio * 10) / 10,
      fiber: Math.round(selected.fi * ratio * 10) / 10,
    });
  }

  function handleManualAdd() {
    if (!manual.name || !manual.calories) return;
    onAdd({ name: manual.name, calories: Number(manual.calories) || 0, protein: Number(manual.protein) || 0, carbs: Number(manual.carbs) || 0, fat: Number(manual.fat) || 0, fiber: Number(manual.fiber) || 0 });
  }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 100, display: "flex", alignItems: "flex-end" }}
      ...
      <div style={{ background: "#0f172a", borderRadius: "20px 20px 0 0", width: "100%", maxHeight: "88vh", overflow: "auto", padding: 20, border: "1px solid #1e293b" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <span style={{ fontSize: 15, fontWeight: 700, color: "#f1f5f9" }}>Adaugă aliment</span>
          <button onClick={onClose} style={{ background: "#1e293b", border: "none", color: "#94a3b8", borderRadius: 8, width: 30, height: 30, fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
        </div>

        {!showManual ? <>
          <input value={query} onChange={e => { setQuery(e.target.value); setSelected(null); }}
            placeholder="🔍 Caută... (pui, orez, iaurt, macrou...)" style={{ ...inp, marginBottom: 10 }} autoFocus />
          ...
          {query.length > 1 && results.length === 0 && (
            <div style={{ background: "#0c1628", borderRadius: 10, padding: 14, marginBottom: 12, border: "1px solid #334155", textAlign: "center" }}>
              <div style={{ fontSize: 13, color: "#64748b", marginBottom: 8 }}>Nu am găsit "{query}" în baza de date</div>
              <button onClick={() => setShowManual(true)} style={{ background: "#1e293b", border: "1px solid #334155", color: "#94a3b8", borderRadius: 8, padding: "7px 14px", fontSize: 12 }}>+ Adaugă manual</button>
            </div>
          )}

          {selected && (
            <div style={{ background: "#0c1628", borderRadius: 10, padding: 12, marginBottom: 12, border: "1px solid #16a34a" }}>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 8 }}>{selected.name}</div>
              <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
                <input type="number" min="1" max="2000" value={grams} onChange={e => setGrams(e.target.value)} style={{ ...inp, width: 90 }} />
                <span style={{ fontSize: 12, color: "#64748b", fontFamily: "monospace" }}>grame</span>
              </div>
              {grams > 0 && (
                <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 10 }}>
                  {[["Cal", Math.round(selected.cal * grams / 100), "#facc15"], ["P", Math.round(selected.p * grams / 100), COLORS.protein], ["C", Math.round(selected.c * grams / 100), COLORS.carbs], ["G", Math.round(selected.f * grams / 100), COLORS.fat], ["F", Math.round(selected.fi * grams / 100), COLORS.fiber]].map(([l, v, c]) => (
                    <span key={l} style={{ background: "#0f172a", borderRadius: 6, padding: "3px 8px", fontSize: 11, fontFamily: "monospace", color: c }}>{l}: {v}</span>
                  ))}
                </div>
              )}
              <button onClick={handleAdd} style={{ width: "100%", padding: "10px", borderRadius: 8, border: "none", background: "linear-gradient(135deg,#16a34a,#4ade80)", color: "#052e16", fontWeight: 700, fontSize: 13 }}>✓ Adaugă</button>
            </div>
          )}

          <button onClick={() => setShowManual(true)} style={{ width: "100%", padding: "9px", borderRadius: 8, border: "1px solid #334155", background: "transparent", color: "#64748b", fontSize: 12, fontFamily: "monospace" }}>
            Nu găsesc alimentul → Adaugă manual
          </button>
        </> : <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {[["Calorii (kcal) *", "calories"], ["Proteine (g)", "protein"], ["Carbohidrați (g)", "carbs"], ["Grăsimi (g)", "fat"], ["Fibre (g)", "fiber"]].map(([ph, key]) => (
                <input key={key} type="number" min="0" placeholder={ph} value={manual[key]} onChange={e => setManual({ ...manual, [key]: e.target.value })} style={{ ...inp, fontSize: 12 }} />
              ))}
            </div>
            <button onClick={handleManualAdd} style={{ padding: "11px", borderRadius: 8, border: "none", background: "linear-gradient(135deg,#16a34a,#4ade80)", color: "#052e16", fontWeight: 700, fontSize: 13 }}>+ Adaugă</button>
          </div>
        </>}
      </div>
    </div>
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
      ...
      <div style={{ background: "#0f172a", borderRadius: 16, width: "100%", maxWidth: 400, padding: 20, border: "1px solid #1e293b" }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
          <span style={{ fontSize: 15, fontWeight: 700 }}>Obiective zilnice</span>
          <button onClick={onClose} style={{ background: "#1e293b", border: "none", color: "#64748b", borderRadius: 8, width: 30, height: 30, fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        ...
        <button onClick={() => { onSave(g); onClose(); }} style={{ marginTop: 14, width: "100%", padding: "11px", borderRadius: 8, border: "none", background: "linear-gradient(135deg,#16a34a,#4ade80)", color: "#052e16", fontWeight: 700, fontSize: 13 }}>
          Salvează
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  function updateDay(fn) {
    setStore(prev => {
      const prevDay = prev.days[today] || emptyDay();
      const newDay = fn(prevDay);
      const updated = { ...prev, days: { ...prev.days, [today]: newDay } };
      saveAll(updated);
      return updated;
    });
  }
  function addWater(ml) { updateDay(d => ({ ...d, water: (d.water || 0) + ml })); }
  function removeWater(ml) { updateDay(d => ({ ...d, water: Math.max(0, (d.water || 0) - ml) })); }
  function deleteMeal(i) { updateDay(d => ({ ...d, meals: d.meals.filter((_, idx) => idx !== i) })); }
  function addMeal(meal) {
    const m = { ...meal, time: new Date().toLocaleTimeString("ro-RO", { hour: "2-digit", minute: "2-digit" }) };
    updateDay(d => ({ ...d, meals: [...d.meals, m] }));
    setShowFood(false);
  }
  ...
  const last30 = Array.from({ length: 30 }, (_, i) => {
    ...
    return { label: d.toLocaleDateString("ro-RO", { day: "numeric", month: "short" }), shortLabel: d.toLocaleDateString("ro-RO", { weekday: "short" }), ...s, water: (store.days[key] || emptyDay()).water || 0 };
  ...
  const advice = getAdvice(totals, goals, dayData);
    ...
    <div style={{ minHeight: "100vh", background: "#080f1a", color: "#e2e8f0", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ padding: "18px 20px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          }}>{totals.calories} / {goals.calories} kcal</div>
        </div>
      </div>
      ...
      <div style={{ padding: "14px 20px 100px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        {/* ── ASISTENT ── */}
        <div style={{ ...C.card, background: "#0a1628", border: "1px solid #1e3a5f" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg,#16a34a,#4ade80)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, flexShrink: 0 }}>🤖</div>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#4ade80", letterSpacing: "0.05em" }}>ASISTENT NUTRIȚIE</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {advice.map((tip, i) => (
              <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", background: "#0c1628", borderRadius: 10, padding: "10px 12px", border: "1px solid #1e293b" }}>
                <span style={{ fontSize: 16, flexShrink: 0 }}>{tip.icon}</span>
                <span style={{ fontSize: 12, color: "#cbd5e1", lineHeight: 1.6 }}>{tip.text}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={C.card}>
          ...
          <div style={{ height: 10, background: "#1e293b", borderRadius: 5, overflow: "hidden" }}>
            <div style={{ height: "100%", width: `${Math.min(calPct, 100)}%`, borderRadius: 5, background: calPct >= 100 ? "#ef4444" : calPct > 85 ? "#f59e0b" : "linear-gradient(90deg,#16a34a,#4ade80)", transition: "width 0.5s ease" }} />
          </div>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 8 }}>
                {[150, 250, 330, 500].map(ml => (
                  <button key={ml} onClick={() => addWater(ml)} style={{ background: "#0c4a6e", border: "1px solid #0369a1", color: "#38bdf8", borderRadius: 7, padding: "5px 9px", fontSize: 11, fontFamily: "monospace" }}>+{ml}</button>
                ))}
                <button onClick={() => removeWater(250)} style={{ background: "#1e293b", border: "1px solid #334155", color: "#64748b", borderRadius: 7, padding: "5px 9px", fontSize: 11, fontFamily: "monospace" }}>−250</button>
              </div>
              <div style={{ display: "flex", gap: 6 }}>
                ...
                <button onClick={() => { if (customWater > 0) { addWater(Number(customWater)); setCustomWater(""); } }}
                  ...
                  style={{ background: customWater > 0 ? "#0c4a6e" : "#1e293b", border: `1px solid ${customWater > 0 ? "#0369a1" : "#334155"}`, color: customWater > 0 ? "#38bdf8" : "#475569", borderRadius: 7, padding: "6px 10px", fontSize: 11, fontFamily: "monospace", whiteSpace: "nowrap" }}>
                  + ml
                </button>
              </div>
            </div>
        ...
        <div style={C.card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <button onClick={() => setShowFood(true)} style={{ background: "linear-gradient(135deg,#16a34a,#4ade80)", border: "none", color: "#052e16", borderRadius: 8, padding: "6px 14px", fontSize: 12, fontWeight: 700 }}>+ Adaugă</button>
          </div>
            ...
            : dayData.meals.map((m, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#0c1628", borderRadius: 10, padding: "10px 12px", marginBottom: 6, border: "1px solid #1e293b" }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 10, color: "#475569", fontFamily: "monospace", marginTop: 2 }}>{m.time} · P:{m.protein}g C:{m.carbs}g G:{m.fat}g F:{m.fiber}g</div>
                </div>
        ...
        <div style={C.card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", gap: 3, flexWrap: "wrap", justifyContent: "flex-end" }}>
              {chartOptions.map(o => (
                <button key={o.key} onClick={() => setChartMetric(o.key)} style={{ padding: "3px 8px", borderRadius: 6, border: "none", fontSize: 10, fontFamily: "monospace", fontWeight: 700, background: chartMetric === o.key ? COLORS[o.key] : "#1e293b", color: chartMetric === o.key ? "#000" : "#475569" }}>{o.label}</button>
              ))}
            </div>
          </div>
          {last30.length > 0 ? (
            <ResponsiveContainer width="100%" height={120}>
              <LineChart data={last30}>
                ...
                <YAxis hide />
                <Tooltip contentStyle={{ background: "#0c1628", border: "1px solid #1e293b", borderRadius: 8, fontSize: 10, fontFamily: "monospace" }} itemStyle={{ color: COLORS[chartMetric] }} formatter={v => [`${v} ${chartOptions.find(o => o.key === chartMetric)?.unit}`, chartOptions.find(o => o.key === chartMetric)?.label]} />
            ...
            <div style={{ height: 80, display: "flex", alignItems: "center", justifyContent: "center", color: "#334155", fontSize: 12 }}>Adaugă mese pentru a vedea evoluția</div>
          )}
        </div>

        {/* ── ISTORIC BAR ── */}
        <div style={C.card}>
          <span style={C.label}>Calorii — ultimele zile</span>
          {last30.length > 0 ? (
            <ResponsiveContainer width="100%" height={130}>
              <BarChart data={last30.slice(-10)} barSize={18}>
                ...
                <ReferenceLine y={goals.calories} stroke="#facc15" strokeDasharray="4 4" strokeOpacity={0.35} />
                <Bar dataKey="calories" fill="#facc15" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <div style={{ height: 80, display: "flex", alignItems: "center", justifyContent: "center", color: "#334155", fontSize: 12 }}>Niciun istoric disponibil</div>
          )}
        </div>
